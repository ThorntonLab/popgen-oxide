...
fn empty_counts() {
    ...
    assert_eq!(counts.num_sample_sets(), 0);
...
fn test_try_reduce_details(
    ...
    let mut mergedcounts = crate::counts::SampleAlleleCounts::of_empty_sample_sets(1);
    ...
    for (i, j) in reduced_counts
        .iter_sample_set(0)
        .unwrap()
        .zip(mergedcounts.iter_sample_set(0).unwrap())
    {
...
#[test]
fn cannot_reduce_different_sample_set_count() {
