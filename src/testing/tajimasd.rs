...
fn tajimas_d() {
    ...
    let d = TajimasD::try_from_iter_sites(allele_counts.iter_population(0).unwrap()).unwrap();
...
proptest!(
    ...
    fn test_try_reduce(
        ...
        // get the calcs
        let diversity_from_counts = TajimasD::try_from_iter_sites(counts.iter_population(0).unwrap());
        if let Ok(value) = diversity_from_counts {
            if !value.as_raw().is_nan() {
                ...
                for nsplits in 0..max_num_splits {
                    ...
                    let div = TajimasD::try_from_iter_sites(counts.iter_population(0).unwrap().skip(nsplits * splitlen).take(takelen)).unwrap_or_default();
                    div_split.push(div);
                }
